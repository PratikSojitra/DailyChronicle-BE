...
export class NewsletterService {
    ...
    public async getNewsletterById(id: string) {
        const newsletter = await this.newsletterRepository.findOne({ where: { id }, relations: ['posts', 'subscribers'] });
        if (!newsletter) {
    ...
    public async createNewsletter(createNewsletterDto: CreateNewsletterDto) {
        const subscribers = await this.subscriberRepository.find({ where: { isActive: true } });
        ...
        const newsletter = this.newsletterRepository.create(createNewsletterPayload);
    ...
    public async subscribe(subscribeDto: SubscribeDto) {
        const subscribers = await this.subscriberRepository.findOne({ where: { email: subscribeDto.email } });
        if (subscribers) {
            const payload = {
                ...subscribeDto,
                updatedAt: new Date()
        ...
        const subscriber = this.subscriberRepository.create(createSubscriberPayload);
        return this.subscriberRepository.save(subscriber);
    }

    public async updateSubscriber(id: string, updateSubscriberDto: UpdateSubscriberDto) {
        const subscriber = await this.subscriberRepository.findOne({ where: { id } });
        if (!subscriber) {
            throw new NotFoundException('Subscriber not found');
        }
