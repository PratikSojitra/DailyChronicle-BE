...
export type EmailType = 'welcome' | 'newsletter';

@Injectable()
export class EmailService {
  private readonly logger = new Logger(EmailService.name);
  private readonly frontendUrl = 'http://localhost:3000'; // Centralized Config

  constructor(private readonly mailerService: MailerService) {}

  // --- SINGLE EMAIL SENDER ---
  async sendEmail(to: string, type: EmailType, context: any = {}) {
    ...
    const { subject, html } = templateFn({ ...context, email: to, frontendUrl: this.frontendUrl });
  ...
  async sendBulkEmail(recipients: string[], type: EmailType, context: any) {
    ...
    this.logger.log(`Starting Bulk Email: ${type} to ${recipients.length} users.`);
